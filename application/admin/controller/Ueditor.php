...
class Ueditor extends Base
{
    ...
    public function index()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (isset($post['title']) && isset($post['content'])) {
                ...
                News::create($news);
                return $this->do_success('发布成功');*/
            }
            return $this->do_error('标题，内容不能为空');
    ...
    public function server()
    {
        ...
        $ue_config = json_decode(preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents(ROOT_PATH . "public/static/admin/plugins/ueditor/php/config.json")), true);
        ...
        if (isset($_GET["callback"])) {
            if (preg_match("/^[\w_]+$/", $_GET["callback"])) {
                ...
                return $this->ajaxReturnData(['state' => 'callback参数不合法']);
            }
        } else {
            return $this->ajaxReturnData($result);
        }
    }
