...
if (!function_exists('parse_padding')) {
    ...
    {
        $length = strlen((string)(count($source['source']) + $source['first']));
...
if (!function_exists('echo_value')) {
    function echo_value($val)
    {
        if (is_array($val) || is_object($val)) {
            echo htmlentities(json_encode($val, JSON_THROW_ON_ERROR | JSON_PRETTY_PRINT));
        } elseif (is_bool($val)) {
            echo $val ? 'true' : 'false';
            ...
            <?php if (!empty($trace['source'])) { ?>
                <div class="source-code">
                    <pre class="prettyprint lang-php"><ol
                                start="<?php echo $trace['source']['first']; ?>"><?php foreach ((array)$trace['source']['source'] as $key => $value) { ?>
                                <li class="line-<?php echo "{$index}-";
                                echo $key + $trace['source']['first'];
                                echo $trace['line'] === $key + $trace['source']['first'] ? ' line-error' : ''; ?>">
                                <code><?php echo htmlentities($value); ?></code></li><?php } ?></ol></pre>
                </div>
            <?php } ?>
            <div class="trace">
...
<?php if (\think\facade\App::isDebug()) { ?>
    <script>
        ...
        ;(function () {
            ...
            (function () {
                let expand = function (dom, expand) {
                    let ol = $('ol', dom.parentNode)[0];
